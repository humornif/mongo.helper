...
namespace Hurricane.Lib.Helper
{
    public abstract class MongoHelper<T> : IDisposable
    {
        private static readonly string _connection_string = ConfigurationManager.ConnectionStrings["MongoConnection"].ConnectionString;
        ...
        protected async Task<long> SelectCountAsync<TField>(Expression<Func<T, TField>> field, TField value)
        {
            ...
            var result = await DBContext.Find(filter).Project(project).CountAsync();
